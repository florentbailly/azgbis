...
Job asynchrone (worker dédié) : ① exécution de l'analyse (mêmes fonctions que `/zones/analyze`) → ② rendu des cartes statiques : instance Playwright/Chromium headless chargeant l'appli carto en mode « rendu » (zone + couche du thème + légende + échelle), une capture PNG par section — le rendu papier est ainsi strictement identique au rendu écran → ③ gabarit HTML/Jinja2 + **WeasyPrint** → PDF paginé A4 portrait (cartes en pleine largeur, paysage pour l'annexe transactions). Objectif : < 3 min par rapport ; 2 jobs concurrents suffisent pour 100 rapports/mois.
